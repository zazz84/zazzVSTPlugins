...
const int SmallSpeakerSimulatorAudioProcessorEditor::SLIDERS[] = { 5 };
const float SmallSpeakerSimulatorAudioProcessorEditor::COLUMN_OFFSET[] = { 0.0f };
...
{	
	// Plugin name
	m_pluginName.setText("Small Speaker Simulator", juce::dontSendNotification);
	...
	for (int i = 0; i < N_SLIDERS; i++)
	{
		...
		const std::string text = SmallSpeakerSimulatorAudioProcessor::paramsNames[i];
		const std::string unit = SmallSpeakerSimulatorAudioProcessor::paramsUnitNames[i];
