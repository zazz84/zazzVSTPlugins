...
const int FilterAudioProcessorEditor::SLIDERS[] = { 4, 2 };
const float FilterAudioProcessorEditor::COLUMN_OFFSET[] = { 0.0f, 1.0f };
...
{	
	// Plugin name
	m_pluginName.setText("Filter", juce::dontSendNotification);
	...
	for (int i = 0; i < N_SLIDERS; i++)
	{
		...
		const std::string text = FilterAudioProcessor::paramsNames[i];
		const std::string unit = FilterAudioProcessor::paramsUnitNames[i];
