...
//const float SmallRoomReverbAudioProcessor::ALLPASS_DELAY_TIMES_MS[] = {8.2f, 21.4f, 29.2f, 38.0f, 49.6f, 66.0};
//const float SmallRoomReverbAudioProcessor::ALLPASS_DELAY_TIMES_MS[] = { 6.56f, 17.12f, 23.36f, 30.40f, 39.68f, 52.80f };
const float SmallRoomReverbAudioProcessor::ALLPASS_DELAY_TIMES_MS[] =	{
																			6.35417f,
																			16.97917f,
																			23.62500f,
																			30.39583f,
...
void SmallRoomReverbAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	m_ERfilter[0].init(sr);
	m_ERfilter[1].init(sr);

	m_LRfilter[0].init(sr);
	m_LRfilter[1].init(sr);

	const int size = (int)(sampleRate * 0.001 * (10.0 + 60.0 + 10.0));
...
void SmallRoomReverbAudioProcessor::releaseResources()
{
	for (int channelReverb = 0; channelReverb < MAX_CHANNELS; channelReverb++)
	{
		for (int allpass = 0; allpass < N_ALLPASSES; allpass++)
		{
			m_allpass[channelReverb][allpass].release();
		}
	}

	m_ERfilter[0].release();
	m_ERfilter[1].release();

	m_LRfilter[0].release();
	m_LRfilter[1].release();
...
void SmallRoomReverbAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	const auto ERsize = 0.1f + 0.9f * ERsizeParameter->load();
	...
	const auto LRgain = juce::Decibels::decibelsToGain(LRvolumeParameter->load() + 11.0f);		// Added output volume conpensation
	...
	const auto APReflectionsTime = 0.05f + 0.95f * LRsize;
	...
	for (int channel = 0; channel < channels; channel++)
	{
		...
		auto& ERfilter = m_ERfilter[channel];
		auto& LRfilter = m_LRfilter[channel];
		auto& earlyReflections = m_earlyReflections[channel];
		auto& allPass = m_allpass[channel];

		ERfilter.set(ERDampingFrequency, ERDampingQ);
		LRfilter.set(LRDampingFrequency, LRDampingQ);
		...
		for (int sample = 0; sample < samples; sample++)
		{
			...
			const float ERout = ERfilter.process(earlyReflections.process(in));

			// Process serial allpass filters
			float LROut = LRfilter.process(earlyReflections.readDelay(LRPredelaySize));

			for (int allpass = 0; allpass < N_ALLPASSES; allpass++)
			{
				LROut = allPass[allpass].process(LROut);
			}
...
juce::AudioProcessorValueTreeState::ParameterLayout SmallRoomReverbAudioProcessor::createParameterLayout()
{
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(paramsNames[4], paramsNames[4], NormalisableRange<float>(   0.0f,  80.0f,  0.01f, 1.0f),   0.0f));
