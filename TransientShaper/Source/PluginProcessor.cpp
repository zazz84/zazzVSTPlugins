...
const std::string TransientShaperAudioProcessor::paramsNames[] = { "AttackLength", "Attack", "SustainLenght", "Sustain", "Volume" };
const std::string TransientShaperAudioProcessor::labelNames[] = { "Length", "Amount", "Lenght", "Amount", "Volume" };
...
#endif
{
	attackLenghtParameter  = apvts.getRawParameterValue(paramsNames[0]);
	attackParameter        = apvts.getRawParameterValue(paramsNames[1]);
	sustainLenghtParameter = apvts.getRawParameterValue(paramsNames[2]);
...
void TransientShaperAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	// Get params
	const auto attackLenght = 0.01f * attackLenghtParameter->load();
	const auto attackGain = 0.01f * attackParameter->load();
	const auto sustainLength = 0.01f * sustainLenghtParameter->load();
	...
	for (int channel = 0; channel < channels; ++channel)
	{
		...
		transientShaper.set(attackGain, sustainGain, attackLenght, sustainLength);

		for (int sample = 0; sample < samples; sample++)
		{
			const float in = channelBuffer[sample];
			
			const float out = transientShaper.process(in);
