...
const int TransientShaperAudioProcessorEditor::SLIDERS[] = { 3 };
const int TransientShaperAudioProcessorEditor::COLUMN_OFFSET[] = { 0 };
...
{	
	// Plugin name
	m_pluginName.setText("Transient Shaper", juce::dontSendNotification);
	...
	for (int i = 0; i < N_SLIDERS; i++)
	{
		...
		const std::string text = TransientShaperAudioProcessor::paramsNames[i];
		const std::string unit = TransientShaperAudioProcessor::paramsUnitNames[i];
