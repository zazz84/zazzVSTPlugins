...
class LimiterTypeLimiterTypeModernRotarySliderTextBox : public juce::Label
{
public:
	LimiterTypeLimiterTypeModernRotarySliderTextBox(const juce::String& componentName, const juce::String& initialText) : juce::Label(componentName, initialText)
	{
...
class LimiterTypeLimiterTypeModernRotarySliderLookAndFeel : public juce::LookAndFeel_V4
{
public:
	enum SliderType
	{
	...
	LimiterTypeLimiterTypeModernRotarySliderLookAndFeel() = default;
...
class LimiterTypeModernRotarySlider : public juce::Component, private juce::Slider::Listener
{
public:

	LimiterTypeModernRotarySlider(juce::AudioProcessorValueTreeState& vts, const std::string name, const std::string unit, const std::string label) : valueTreeState(vts), m_name(name), m_unit(unit), m_label(label)
	{
		...
		m_textBox.setEditable(false, false, false);														// Allow editing
		
		const auto value = (int)m_slider.getValue();
		if (value == 1)
		{
			m_textBox.setText("Dirty", juce::dontSendNotification);
		}
		else if (value == 2)
		{
			m_textBox.setText("Agressive", juce::dontSendNotification);
		}
		else
		{
			m_textBox.setText("Clean", juce::dontSendNotification);
		}

		m_textBox.onTextChange = [this]() { handleTextBoxChange(); };									// Update slider on label change
		addAndMakeVisible(m_textBox);
	};
	~LimiterTypeModernRotarySlider()
	{
	...
	void setSliderType(LimiterTypeLimiterTypeModernRotarySliderLookAndFeel::SliderType sliderType)
	{
...
private:
	...
	LimiterTypeLimiterTypeModernRotarySliderLookAndFeel m_smallSliderLookAndFeel;

	LimiterTypeLimiterTypeModernRotarySliderTextBox m_textBox{ "CustomLabel", "Default Text" };
