...
const int DeharshAudioProcessorEditor::SLIDERS[] = { 5 };
const float DeharshAudioProcessorEditor::COLUMN_OFFSET[] = { 0.0f };
...
{	
	// Plugin name
	m_pluginName.setText("De-harsh", juce::dontSendNotification);
	...
	for (int i = 0; i < N_SLIDERS; i++)
	{
		...
		const std::string text = DeharshAudioProcessor::paramsNames[i];
		const std::string unit = DeharshAudioProcessor::paramsUnitNames[i];
